    ...
    <div className="mx-auto max-w-6xl px-6 py-12 lg:px-8">
      <div className="mb-10">
        <div className="flex items-center gap-3">
          <Link to="/dashboard" className="inline-flex items-center gap-1 text-[15px] font-medium text-[#6B7280] transition-colors hover:text-[#1A1A2E]">
          ...
          <span className="text-[15px] font-medium text-[#1A1A2E]">Best Matches</span>
        </div>
        <h1 className="mt-5 text-[40px] font-bold leading-[1.15] tracking-[-0.03em] text-[#1A1A2E]">
      ...
      {hasSkills && (
        <>
          <div className="mb-8 grid gap-5 sm:grid-cols-4">
            {[
              { label: "Total Matches", value: String(filteredMatches.length), color: "text-[#1A1A2E]" },
              { label: "Average Match", value: `${avgScore}%`, color: "text-[#1A1A2E]" },
          ...
          <div className="mb-8 flex flex-wrap items-center gap-3">
            <select value={selectedProvince} onChange={(e) => setSelectedProvince(e.target.value)} className="rounded-2xl border border-[#E5E7EB] bg-white px-4 py-2.5 text-[15px] text-[#1A1A2E] focus:border-[#2563EB] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/10">
              <option value="">All Provinces</option>
              {provinces.map((p) => (<option key={p} value={p}>{p}</option>))}
            </select>
            <select value={selectedIndustry} onChange={(e) => setSelectedIndustry(e.target.value)} className="rounded-2xl border border-[#E5E7EB] bg-white px-4 py-2.5 text-[15px] text-[#1A1A2E] focus:border-[#2563EB] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/10">
              <option value="">All Industries</option>
              {industries.map((ind) => (<option key={ind} value={ind}>{ind}</option>))}
            </select>
            <div className="flex gap-1.5">
              {(["all", "strong", "good", "low"] as const).map((f) => (
                <button key={f} onClick={() => setScoreFilter(f)} className={`rounded-2xl px-4 py-2.5 text-sm font-semibold transition-colors ${scoreFilter === f ? "bg-[#2563EB] text-white" : "border border-[#E5E7EB] bg-white text-[#4B5563] hover:bg-white"}`}>
                  {f === "all" ? "All Scores" : f === "strong" ? "80%+" : f === "good" ? "50-79%" : "<50%"}
                </button>
              ))}
            </div>
            {(selectedProvince || selectedIndustry || scoreFilter !== "all") && (
              <button onClick={() => { setSelectedProvince(""); setSelectedIndustry(""); setScoreFilter("all"); }} className="rounded-2xl px-4 py-2.5 text-sm font-medium text-[#6B7280] transition-colors hover:bg-white hover:text-[#1A1A2E]">
                Clear Filters
              </button>
            )}
          </div>
          ...
          {filteredMatches.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <h3 className="mt-6 text-xl font-bold text-[#1A1A2E]">No matches found</h3>
            ...
            <div className="space-y-5">
              {filteredMatches.map((match) => {
                ...
                return (
                  <div key={match.job.id} className="overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white ">
                    <div className="flex flex-col gap-5 p-8 sm:flex-row sm:items-center sm:justify-between">
                      <div className="flex items-start gap-5">
                        ...
                        <div>
                          {isFreeUser ? (
                            <a href="/pricing" className="text-xl font-bold text-[#1A1A2E] transition-colors hover:text-[#2563EB]">
                              {match.job.title}
                            </a>
                          ) : (
                            <Link to="/jobs/$jobId" params={{ jobId: match.job.id }} className="text-xl font-bold text-[#1A1A2E] transition-colors hover:text-[#2563EB]">
                              {match.job.title}
                            </Link>
                          )}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => setExpandedMatch(isExpanded ? null : match.job.id)} className="rounded-2xl border border-[#E5E7EB] bg-white px-5 py-2.5 text-sm font-medium text-[#1A1A2E] transition-colors hover:bg-white">
                    ...
                    {isExpanded && (
                      <div className="border-t border-[#E5E7EB] bg-white p-8">
                        <div className="grid gap-8 lg:grid-cols-2">
                          <div>
                            <h4 className="mb-4 text-sm font-semibold text-[#1A1A2E]">Match Breakdown</h4>
                              ...
                              <div>
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                  {match.missingSkills.map((s, i) => (
                                    <span key={i} className="inline-flex items-center rounded-full bg-[#F3F4F6] px-3 py-1 text-xs font-medium text-[#6B7280]">{s}</span>
                                  ))}
                                </div>
                              </div>
                            )}
