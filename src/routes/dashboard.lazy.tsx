...
function DashboardContent() {
  ...
  return (
    <div className="flex">
      ...
      <aside className={`fixed top-[73px] left-0 z-50 h-[calc(100dvh-73px)] w-64 transform border-r border-[#E5E7EB] bg-white transition-transform duration-200 lg:sticky lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-full flex-col">
          <div className="border-b border-[#E5E7EB] px-6 py-6">
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#EFF6FF] text-sm font-bold text-[#1A1A2E]">{userName.charAt(0).toUpperCase()}</div>
              <div className="min-w-0">
                <p className="truncate text-[15px] font-semibold text-[#1A1A2E]">{userName}</p>
          ...
          <nav className="flex-1 space-y-1 px-4 py-5">
            {navItems.map((item) => (
              <Link key={item.label} to={item.to} onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 rounded-2xl px-4 py-3 text-[15px] font-medium text-[#4B5563] transition-colors hover:bg-white hover:text-[#1A1A2E] [&.active]:bg-[#F3F4F6] [&.active]:text-[#1A1A2E]">
                {item.icon}{item.label}
              </Link>
            ))}
          </nav>
          <div className="border-t border-[#E5E7EB] px-4 py-5">
            <Link to="/employers" className="flex items-center gap-3 rounded-2xl px-4 py-3 text-[15px] font-medium text-[#4B5563] transition-colors hover:bg-white hover:text-[#1A1A2E]">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3 border-b border-[#E5E7EB] bg-white px-6 py-4 lg:hidden">
          <button onClick={() => setSidebarOpen(true)} className="rounded-xl p-2 text-[#4B5563] hover:bg-white" aria-label="Open sidebar">
          ...
          <span className="text-[15px] font-semibold text-[#1A1A2E]">Dashboard</span>
        </div>
        ...
        <div className="px-6 py-10 lg:px-10">
          ...
          <div className="mb-10">
            <h1 className="text-[36px] font-bold tracking-[-0.03em] text-[#1A1A2E]">Welcome back, {userName}!</h1>
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            <div className="space-y-8 lg:col-span-2">
              <div className="rounded-2xl border border-[#E5E7EB] bg-white p-8 ">
                <div className="mb-6 flex items-center justify-between">
                  <h2 className="text-xl font-bold text-[#1A1A2E]">Top Job Matches</h2>
                ...
                {hasSkills && topMatches.length > 0 && (
                  <div className="grid gap-4 sm:grid-cols-2">
                    {topMatches.map((match) => {
                      ...
                      return (
                        <Link key={match.job.id} to="/jobs/$jobId" params={{ jobId: match.job.id }} className="group block rounded-2xl border border-[#E5E7EB] bg-white p-5 hover:">
                          <div className="flex items-start gap-3">
                            <div className="flex-shrink-0"><MatchScoreBadge score={match.matchScore} size="sm" /></div>
                            <div className="min-w-0 flex-1">
                              <h3 className="text-sm font-semibold text-[#1A1A2E] transition-colors group-hover:text-[#2563EB]">{match.job.title}</h3>
                              <p className="mt-1 text-xs text-[#6B7280]">{emp?.name || "Employer"}</p>
                              <div className="mt-2 flex flex-wrap gap-1.5">
                                <span className="inline-flex items-center rounded-full bg-[#F3F4F6] px-2.5 py-0.5 text-xs text-[#4B5563]">{match.job.location}</span>
              ...
              <div className="rounded-2xl border border-[#E5E7EB] bg-white p-8 ">
                <div className="mb-6 flex items-center justify-between">
                  <h2 className="text-xl font-bold text-[#1A1A2E]">Saved Jobs</h2>
                ...
                {savedJobsList.length > 0 ? (
                  <div className="grid gap-4 sm:grid-cols-2">
                    {savedJobsList.slice(0, 6).map((job) => {
                      ...
                      return (
                        <Link key={job.id} to="/jobs/$jobId" params={{ jobId: job.id }} className="group block rounded-2xl border border-[#E5E7EB] bg-white p-5 hover:">
                          <h3 className="text-sm font-semibold text-[#1A1A2E] transition-colors group-hover:text-[#2563EB]">{job.title}</h3>
                          <p className="mt-1 text-xs text-[#6B7280]">{emp?.name || "Employer"} &middot; {job.location}</p>
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            <span className="inline-flex items-center rounded-full bg-[#F3F4F6] px-2.5 py-0.5 text-xs text-[#4B5563]">{job.type}</span>
                            <span className="inline-flex items-center rounded-full bg-white px-2.5 py-0.5 text-xs text-[#6B7280]">{job.salary}</span>
                          </div>
                        </Link>
                      );
                    })}
              ...
              <div className="rounded-2xl border border-[#E5E7EB] bg-white p-8 ">
                <h2 className="mb-6 text-xl font-bold text-[#1A1A2E]">Recent Activity</h2>
                <div className="space-y-5">
                  {[
                    ...
                    <div key={item.title} className="flex items-start gap-3">
                      <div className={`mt-1 flex h-3 w-3 flex-shrink-0 rounded-full ${item.active ? "bg-[#2563EB]" : "bg-[#F3F4F6]"}`} />
            ...
            <div className="space-y-8">
              <div className="rounded-2xl border border-[#E5E7EB] bg-white p-8 ">
                <h2 className="text-sm font-semibold text-[#1A1A2E]">Resume AI Score</h2>
                {profile?.resume_text ? (
                  <div className="mt-6 text-center">
                    <div className="inline-flex h-24 w-24 items-center justify-center rounded-full bg-[#F3F4F6]"><span className="text-3xl font-bold text-[#6B7280]">—</span></div>
                    <p className="mt-4 text-sm font-medium text-[#1A1A2E]">Resume uploaded</p>
                    <p className="mt-1 text-xs text-[#6B7280]">AI scoring coming soon</p>
                  </div>
                ) : (
                  <div className="mt-6 text-center">
                    <div className="inline-flex h-24 w-24 items-center justify-center rounded-full bg-[#F3F4F6]">
                    ...
                    <p className="mt-4 text-sm font-medium text-[#1A1A2E]">Upload your resume to get an AI score</p>
              ...
              <div className="rounded-2xl border border-[#E5E7EB] bg-white p-8 ">
                <h2 className="text-sm font-semibold text-[#1A1A2E]">Profile Completeness</h2>
                <div className="mt-6">
                  <div className="flex items-center justify-between">
                    <span className="text-4xl font-bold text-[#1A1A2E]">{completeness}%</span>
                    <span className="text-sm text-[#6B7280]">complete</span>
                  </div>
                  <div className="mt-4 h-2.5 w-full rounded-full bg-[#F3F4F6]"><div className="h-2.5 rounded-full bg-[#2563EB] duration-500" style={{ width: `${completeness}%` }} /></div>
              ...
              <div className="rounded-2xl border border-[#E5E7EB] bg-white p-8 ">
                <h2 className="text-sm font-semibold text-[#1A1A2E]">Quick Tips</h2>
                <ul className="mt-5 space-y-4">
                  {[
                    ...
                    <li key={i} className="flex gap-3 text-sm text-[#6B7280]">
                      <span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-[#F3F4F6] text-xs font-bold text-[#4B5563]">{i + 1}</span>
                      {tip}
                    </li>
                  ))}
                </ul>
              ...
              <div className="rounded-2xl border border-[#E5E7EB] bg-white p-8 ">
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-semibold text-[#1A1A2E]">Job Alerts</h2>
                </div>
                <p className="text-sm text-[#6B7280]">
                  We&apos;ll notify you when new jobs match your profile and preferences.
                </p>
                <div className="mt-4">
                  <div className="flex items-center gap-2 rounded-2xl border border-[#E5E7EB] bg-white px-4 py-3">
...
function Unauthenticated() {
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center px-6 text-center">
      ...
      <h2 className="mt-8 text-[28px] font-bold text-[#1A1A2E]">Sign In Required</h2>
      ...
      <div className="mt-8 flex items-center gap-3">
        <SignInButton mode="modal">
          <button type="button" className="rounded-2xl border border-[#E5E7EB] bg-white px-6 py-3 text-[16px] font-semibold text-[#1A1A2E] transition-colors hover:bg-white">Sign In</button>
        </SignInButton>
